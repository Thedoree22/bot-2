...
class TikTokCog(commands.Cog):
    ...
    async def add_tiktok(self, interaction: discord.Interaction, username: str, discord_channel: discord.TextChannel):
        ...
        try:
             ...
             if response.status_code != 200:
                  await interaction.response.send_message(f"Ver vipove akkaunti saxelit '{username}'.", ephemeral=True); return
        except requests.exceptions.RequestException as e:
            print(f"Error checking TikTok user {username}: {e}")
            await interaction.response.send_message("Ver davukavshirdi TikTok-s akkauntis shesamowmeblad.", ephemeral=True); return

        self.tiktok_data[guild_id]["channels"][username.lower()] = {
            "discord_channel_id": discord_channel.id, "last_post_id": None,
            "is_live": False, "mention": None
        }
        save_tiktok_data(self.tiktok_data)
        await interaction.response.send_message(f"TikTok akkaunti `{username}` damatebulia #{discord_channel.name}-shi.", ephemeral=True)
    ...
    @app_commands.describe(username="TikTok akkauntis saxeli", mention_everyone="Moinishnos Tu Ara @everyone?", role="Airchiet roli romelic moinishneba (Tu @everyone ar ginda)")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def set_mention_tiktok(self, interaction: discord.Interaction, username: str, mention_everyone: bool = False, role: Optional[discord.Role] = None):
        ...
        if mention_everyone:
            mention_target = "everyone"; message = f"`{username}`-is laivze `@everyone` moinishneba."
        elif role:
            mention_target = str(role.id); message = f"`{username}`-is laivze {role.mention} moinishneba."
        else:
            mention_target = None; message = f"`{username}`-is laivze aravin moinishneba."
    ...
    async def force_live_tiktok(self, interaction: discord.Interaction, username: str):
        ...
        if not discord_channel:
            await interaction.response.send_message("ver vipove Discord arkhi.", ephemeral=True); return
        ...
        try:
            ...
            if is_live_now:
                ...
                mention_content = None; mention_everyone_bool = False; role_mention = False
                if mention_setting == "everyone":
                    mention_content = "@everyone"; mention_everyone_bool = True
                ...
                await discord_channel.send(f"🔴 **ლაივია!** `{username}` TikTok-ze isev laivshia!\n{url}", content=mention_content, allowed_mentions=discord.AllowedMentions(everyone=mention_everyone_bool, roles=role_mention))
    ...
    async def check_tiktok(self):
        ...
        for guild_id, guild_data in current_data.items():
            ...
            for username, config in list(guild_data.get("channels", {}).items()): # ვიყენებთ list()-ს, რომ წაშლა არ შეგვიშალოს
                discord_channel_id = config.get("discord_channel_id")
                ...
                try:
                    ...
                    response = requests.get(url, headers=self.headers, timeout=15)
                    response.raise_for_status() # ამოწმებს 404 და სხვა HTTP შეცდომებს
                    soup = BeautifulSoup(response.text, 'html.parser')

                    # 1. ვიდეოების შემოწმება
                         ...
                         if post_link_tag and post_link_tag.get('href'):
                            ...
                            if match:
                                ...
                                if last_known_id is None:
                                    ...
                                    # პირველ ჯერზე არაფერს ვაგზავნით, უბრალოდ ვინახავთ
                                elif last_known_id != latest_post_id:
                                    current_data[guild_id]["channels"][username]["last_post_id"] = latest_post_id
                                    await channel.send(f"📢 **ახალი TikTok ვიდეო!** `{username}`-მა დადო ახალი პოსტი:\n{post_url}")
                                    save_tiktok_data(current_data) # ვინახავთ მხოლოდ გაგზავნისას

                    # 2. ლაივის შემოწმება
                    is_live_now = "live" in response.text.lower() # ეს შემოწმება შეიძლება არ იყოს 100% ზუსტი
                    was_live = config.get("is_live", False)

                    if is_live_now and not was_live:
                         current_data[guild_id]["channels"][username]["is_live"] = True
                         save_tiktok_data(current_data) # ვინახავთ სტატუსის ცვლილებას
                ...
                # --- აქ არის სწორად დამატებული except ბლოკები ---
                     ...
                     if e.response.status_code == 404:
                         ...
                         if guild_id in current_data and username in current_data.get(guild_id, {}).get("channels", {}):
                             ...
                             save_tiktok_data(current_data) # ვინახავთ წაშლის შემდეგ
                     else:
                          print(f"HTTP Error checking TikTok {username}: {e.response.status_code}") # ვბეჭდავთ სტატუს კოდს
                except requests.exceptions.RequestException as e: # იჭერს კავშირის შეცდომებს, ტაიმაუტებს და ა.შ.
                    print(f"Connection Error checking TikTok {username}: {e}")
                except Exception as e: # იჭერს ყველა სხვა დანარჩენ შეცდომას
