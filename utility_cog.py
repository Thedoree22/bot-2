...
class UtilityCog(commands.Cog):
    ...
    # --- Giveaway-ს შემმოწმებელი (მონაწილეთა რაოდენობით) ---
    ...
    async def check_giveaways(self):
        ...
        for msg_id, data in list(giveaways.items()):
            ...
            if current_time >= end_time:
                ...
                try: # ვცადოთ ძველი შეტყობინების ედიტი, თუ შეცდომა მოხდა, უბრალოდ გავატაროთ
                    ...
                    if len(original_embed.fields) > 1: original_embed.remove_field(1)
                    original_embed.add_field(name="მონაწილე:", value=f"{participant_count} კაცი", inline=True)
                    original_embed.add_field(name="გამარჯვებული:", value=winner_text if winners_list else "არავინ", inline=True)
    ...
    # --- ფონური პროცესი (1 საათიანი ინტერვალით, გასწორებული Indentation) ---
    @tasks.loop(hours=5)
    async def send_auto_message(self):
        await self.bot.wait_until_ready() # ეს უნდა იყოს ფუნქციის შიგნით
        data = load_data(AUTOMESSAGE_DB)
        for guild_id, config in data.items():
            if 'channel_id' in config:
                channel = self.bot.get_channel(config['channel_id'])
                if channel:
                    await self._send_the_message(channel)
                else:
                    print(f"WARNING: ვერ მოიძებნა auto-msg არხი ID={config['channel_id']} სერვერისთვის ID={guild_id}") # დავამატეთ სერვერის ID
