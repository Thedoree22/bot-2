...
class GiveawayView(discord.ui.View): # ... (კოდი იგივეა) ...
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not giveaway or giveaway.get('ended', False): await interaction.response.send_message("ეს გათამაშება დასრულდა ან აღარ არსებობს.", ephemeral=True); return # დავამატეთ ended შემოწმება
        user_id = str(interaction.user.id)
        if user_id not in giveaway['participants']: giveaway['participants'].append(user_id); save_data(giveaways, GIVEAWAY_DB); await interaction.response.send_message("✅ წარმატებით ჩაერთე", ephemeral=True) # შევცვალეთ პასუხი
        else: await interaction.response.send_message("⚠️ შენ უკვე მონაწილეობ", ephemeral=True) # შევცვალეთ პასუხი
...
class UtilityCog(commands.Cog):
    ...
    def log_sms(self, user_id: int, direction: str, content: str, admin_id: Optional[int] = None): #...(SMS ლოგირება უცვლელი)...
    ...
    # --- სხვა ბრძანებები (Clear, Giveaway Start, Userinfo, Join, Leave, daketva, gageba, auto-msg, sms, smslog) ---
    ...
    @app_commands.command(name="giveaway", description="ქმნის ახალ გათამაშებას") # ... (Giveaway Start კოდი) ...
    ...
    async def view_sms_log(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not log_text: log_text = "ლოგები ვერ მოიძებნა."; embed.description = log_text; embed.set_footer(text=f"ნაჩვენებია ბოლო {len(
